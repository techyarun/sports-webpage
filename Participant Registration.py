...
cur = conn.cursor()
q1 = """select max(id) from participant"""
cur.execute(q1)
r=cur.fetchone()
if r[0]!=None:
    n=r[0]
...
ptid = "ptid" + z + str(n+1)                                                                #common id with an certain condition
print("""                                                                                   #html and css
<!DOCTYPE html>
<html lang="en">
<head>
...
<body>
    <h1>Players Registration</h1>                                                                       #registration field details
...
print("""
        ...
        <input type ="button" value ="cancel" onclick = "location.href = 'participantlogin.py'">
    </form>
</body>
</html>
  """ %(ptid))
...
if sub !=None:                                                                                              #to get the all fields value
    ...
    if profile != None:
        if profile.filename:
            ...
            q = """insert into participant(ptid,name,gender,email,contact,age,game,bloodgroup,username,password,photo) values('%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s')""" %(ptid,name,gender,email,contact,age,game,bloodgroup,username,password,fp)
            cur.execute(q)
            conn.commit()
            conn.close()


            print("""                                                                           #alert message when data inserted
                <script>
                    alert("Data inserted successfully");
                    location.href = "participantlogin.py"
                </script>
            """)
    else:
