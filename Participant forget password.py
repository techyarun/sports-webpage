...
<h3>Forget Password</h3>
    <form method = "post" action = "#">                            #to get username 
        Username:
        <input type = "text" name = "username"><br><br>
        <input type = "submit" value = "OK" name = "submit"> <input type ="button" value ="cancel" onclick = "location.href = 'participantlogin.py'">
        </form>
        </center>
        </body>
        </html>""")
...
if v != None:
    q = """select password ,email from participant where username = '%s'"""%(username)          #selecting username and password from sql
    cur.execute(q)
    r = cur.fetchone()
    ...
    if r[0] != None:
        ...
        print("""
            <script>
            alert("Please check your mail");                                                    #shows alert message 
